...
def main():
    ...
    parser.add_argument('--tic_id', type=int, required=True, help='TIC ID of the target star to process')
    ...
    for phot_file in phot_files:
        ...
        if cam_bin == 2:
            ...
            APERTURE = 20  # default fallback

        logger.info(f'The parameters are: APERTURE={APERTURE}')

        base_filename = phot_file.split('.')[0]
        fits_filename = f"rel_{base_filename}_{APERTURE}_{bin_size}_{target_tic_id}.fits"
        if os.path.exists(fits_filename):
            logger.info(f"Data for TIC {target_tic_id} already saved to {fits_filename}. Skipping analysis.")
            continue

        # --- Filter table for this TIC ID ---
        if target_tic_id not in phot_table['tic_id']:
            logger.warning(f"TIC ID {target_tic_id} not found in {phot_file}. Skipping file.")
            continue

        mask = phot_table['tic_id'] == target_tic_id
        tmag_value = phot_table['Tmag'][mask][0]  # Take the first Tmag for this star
        print(f"\033[94m\nRunning TIC {target_tic_id} (Tmag={tmag_value:.3f})\033[0m")  # Blue text
        ...
        for dmb, dmf, crop_size, color_tol in itertools.product(dmb_values, dmf_values, crop_values, color_values):
            print(f"Testing TIC {target_tic_id} with parameters: dmb={dmb}, dmf={dmf}, crop={crop_size}, color_tol={color_tol}")
            global COLOR_TOLERANCE
            COLOR_TOLERANCE = color_tol

            result = relative_phot(phot_table, target_tic_id, bin_size, APERTURE, dmb, dmf, crop_size)
            if result is None:
                continue
            ...
            if rms < best_rms:
                ...
                print('\n')
                print(f"\033[92m--> New best RMS: {best_rms:.6f} with dmb={dmb}, dmf={dmf}, crop={crop_size}, color_tol={color_tol}\033[0m")
                print('\n')
        if best_result is None:
            logger.info(f"TIC ID {target_tic_id} skipped: no valid parameter combination found.")
            continue

        logger.info(f"Best RMS for TIC {target_tic_id}: {best_rms:.6f} with params {best_params}")

        (target_tmag, time_binned, dt_flux_binned, dt_fluxerr_binned, sky_median,
         airmass, zp, color) = best_result

        data_table = Table(
            rows=[(target_tic_id, target_tmag, time_binned, dt_flux_binned, dt_fluxerr_binned,
                   sky_median, best_rms, airmass, zp, color)],
            names=('TIC_ID', 'Tmag', 'Time_BJD', 'Relative_Flux',
        ...
        logger.info(f"Data for TIC {target_tic_id} saved to {fits_filename}.")
